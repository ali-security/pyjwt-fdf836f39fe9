...
class TestKeyLengthValidation:
    ...
    def test_pyjws_decode_warns_short_hmac_key(self):
        ...
        with pytest.warns(jwt.InsecureKeyLengthWarning, match="below"):
            jws.decode(token, b"short", algorithms=["HS256"])
    ...
    def test_pyjws_decode_enforces_short_hmac_key(self):
        ...
        token = jwt.PyJWS().encode(
            b'{"test":"payload"}', adequate_key, algorithm="HS256"
        )
    ...
    @crypto_required
    def test_pyjwt_encode_warns_small_rsa_key(self):
        from cryptography.hazmat.primitives.asymmetric import rsa as rsa_module
        ...
        with pytest.warns(jwt.InsecureKeyLengthWarning, match="2048"):
            jwt.encode({"hello": "world"}, small_key, algorithm="RS256")

    @crypto_required
    def test_pyjwt_encode_enforces_small_rsa_key(self):
        from cryptography.hazmat.primitives.asymmetric import rsa as rsa_module
        ...
        with pytest.raises(InvalidKeyError, match="2048"):
            pyjwt.encode({"hello": "world"}, small_key, algorithm="RS256")

    def test_default_options_do_not_enforce_key_length(self):
        import jwt
    ...
    def test_global_register_algorithm_visible_to_global_encode(self):
        """jwt.register_algorithm must affect jwt.encode (shared PyJWS instance)."""
        import jwt

        jwt.register_algorithm("CUSTOM_NONE", NoneAlgorithm())
        try:
            token = jwt.encode({"hello": "world"}, None, algorithm="CUSTOM_NONE")
            assert token.count(".") == 2
        finally:
            jwt.unregister_algorithm("CUSTOM_NONE")
